...
function change_ddns_setting(v){
		...
		if (v == "WWW.ASUS.COM"){
			...
			inputCtrl(document.form.ddns_regular_period, 0);			
				...
				inputCtrl(document.form.ddns_username_x, 1);
				inputCtrl(document.form.ddns_passwd_x, 1);
				if(v == "WWW.TUNNELBROKER.NET" || v == "WWW.SELFHOST.DE" || v == "WWW.GOOGLE-DOMAINS.COM")
					var disable_wild = 1;
				else
					var disable_wild = 0;
				document.form.ddns_wildcard_x[0].disabled= disable_wild;
				document.form.ddns_wildcard_x[1].disabled= disable_wild;
				if(v == "WWW.ZONEEDIT.COM" || v == "WWW.GOOGLE-DOMAINS.COM"){
					showhide("link", 0);
...
function openLink(s){
	if (s == 'x_DDNSServer'){
		...
		else if (document.form.ddns_server_x.value == 'WWW.GOOGLE-DOMAINS.COM')
