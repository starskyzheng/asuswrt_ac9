 ...
 *
 * $Id: bcmwpa.c 542540 2015-03-20 04:44:40Z $
 */
...
void
wpa_calc_pmkid(struct ether_addr *auth_ea, struct ether_addr *sta_ea,
...
uint8 *
wpa_array_cmp(int max_array, uint8 *x, uint8 *y, uint len)
{
	uint i;
	uint8 *ret = x;
...
bcmwpa_includes_wpa2_auth(uint32 wpa_auth)
{
	...
	if (auth & (WPA2_AUTH_UNSPECIFIED |
		WPA2_AUTH_PSK |
#ifndef SHA256_ROM_COMPAT
		WPA2_AUTH_1X_SHA256| WPA2_AUTH_PSK_SHA256 |
#endif
		BRCM_AUTH_PSK))
		return TRUE;
	else
